...
class NewsMgr{
    ...
    long addNews(News info){
        ...
        return writableDatabase.insertWithOnConflict(SqliteHelper.NEWS_TAB, null, values,
    ...
    void addNewsList(List<News> list){
        ...
        try{
            for(News info : list){
                ContentValues values = buildNewsInfo(info);
                writableDatabase.insertWithOnConflict(SqliteHelper.NEWS_TAB, null, values,
                                                      SQLiteDatabase.CONFLICT_REPLACE);
            }
            // 数据插入操作循环
    ...
    News getNewsByID(int id){
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM " + SqliteHelper.NEWS_TAB + " WHERE " + News.NEWS_SERVER_ID + " = "
                + id + "", null);
    ...
    List<News> getAllNews(){
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + SqliteHelper.NEWS_TAB + " ORDER BY " + News.TIMESTAMP + " DESC",
                                    null);
    ...
    List<News> getAllNewsByType(int type){
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + SqliteHelper.NEWS_TAB + " WHERE " + News.NEWS_TYPE + " = "
                + type + " ORDER BY " + News.TIMESTAMP + " DESC", null);
    ...
    List<News> getNewsByType(int type, int max){
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + SqliteHelper.NEWS_TAB + " WHERE " + News.NEWS_TYPE + " = "
                + type + " ORDER BY " + News.TIMESTAMP + " DESC LIMIT " + max, null);
    ...
    List<News> getNews(int max, long timestamp){
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + SqliteHelper.NEWS_TAB + " WHERE " + News.TIMESTAMP + " < "
                + timestamp + " ORDER BY " + News.TIMESTAMP + " DESC LIMIT " + max, null);
    ...
    void removeAllNewsByType(int type){
        ...
        db.execSQL("DELETE FROM " + SqliteHelper.NEWS_TAB + " WHERE " + News.NEWS_TYPE + " = " + type);
    }
