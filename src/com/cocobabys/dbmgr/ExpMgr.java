...
class ExpMgr{
    ...
    List<ExpInfo> getExpInfoByMonthAndYear(String monthAndYear){
        ...
        Cursor cursor = db.rawQuery("SELECT * " + " FROM " + SqliteHelper.EXP_TAB + " WHERE "
                + " STRFTIME('%Y-%m', timestamp / 1000, 'unixepoch') " + "='" + monthAndYear + "'" + " AND "
                + ExpInfo.CHILD_ID + " = '" + childid + "' ORDER BY timestamp DESC", null);
    ...
    void deleteExp(long expid){
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        db.execSQL("DELETE FROM " + SqliteHelper.EXP_TAB + " WHERE " + ExpInfo.EXP_ID + " = " + expid);
    }
