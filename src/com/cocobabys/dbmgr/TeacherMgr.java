...
public class TeacherMgr{
    ...
    private ContentValues setProp(Teacher info){
        ...
        values.put(Teacher.INTERNAL_ID, info.getInternal_id());
        values.put(Teacher.LOGIN_NAME, info.getLogin_name());
        return values;
    }
    ...
    boolean handleIncomingTeacher(Teacher fromnet){
        ...
        if(localone == null){
            add(fromnet);
            bret = true;
        } else if(fromnet.getTimestamp() > localone.getTimestamp() || TextUtils.isEmpty(localone.getLogin_name())
                || localone.getInternal_id() == -1){
            add(fromnet);
            bret = true;
        }
    ...
    Teacher getTeacherByInternalID(int internalid){
        Teacher info = null;
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + SqliteHelper.TEACHER_TAB + " WHERE " + Teacher.INTERNAL_ID
                + " = " + internalid, null);
        try{
            cursor.moveToFirst();
    ...
    private Teacher getInfoByCursor(Cursor cursor){
        ...
        info.setInternal_id(cursor.getInt(11));
        info.setLogin_name(cursor.getString(12));
        return info;
    }
