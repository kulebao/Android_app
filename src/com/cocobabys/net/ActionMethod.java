...
public class ActionMethod {
	...
	private MethodResult getInfoResult(HttpResult result) throws Exception {
		...
		if (result.getResCode() == HttpStatus.SC_OK) {
			List<ActionInfo> list = JSON.parseArray(result.getContent(),
					ActionInfo.class);
	...
	private String createGetInfoCommand(PullToRefreshListInfo info) {
		String cmd = String.format(ServerUrls.GET_ACTION_LIST, DataMgr
	...
	private MethodResult getCheckEnrollResult(HttpResult result) {
		Log.d("DDD ActionMethod getEnrollResult",
				" result : " + result.toString());
		MethodResult methodResult = new MethodResult(
				EventType.ACTION_GET_ENROLL_FAIL);
		// 成功就表示报过名了，否则返回404
		if (result.getResCode() == HttpStatus.SC_OK) {
	...
	private String createGetEnrollStateCommand(int actionID) {
		String cmd = String.format(ServerUrls.GET_ENROLL_STATE, DataMgr
				.getInstance().getSchoolID(), actionID, DataMgr.getInstance()
	...
	private MethodResult getDoEnrollResult(HttpResult result) {
		Log.d("DDD ActionMethod getDoEnrollResult",
				" result : " + result.toString());
		MethodResult methodResult = new MethodResult(
				EventType.ACTION_DO_ENROLL_FAIL);
		if (result.getResCode() == HttpStatus.SC_OK) {
	...
	private String getDoEnrollContent(ActionInfo actionInfo)
	...
	private String createDoEnrollCommand(int actionID) {
		String cmd = String.format(ServerUrls.DO_ENROLL, DataMgr.getInstance()
				.getSchoolID(), actionID);
		return cmd;
	}
}
