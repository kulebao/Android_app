...
public class NewChatActivity extends UmengStatisticsActivity{
    ...
    public void onCreate(Bundle savedInstanceState){
        ...
        setContentView(R.layout.new_chat_pull_refresh_list);
        ...
        camera.setOnClickListener(new MyClickListner(){
            ...
            public void execute(){
                if(!Utils.isSdcardExisting()){
                    Toast.makeText(NewChatActivity.this, "未找到存储卡，无法保存图片！", Toast.LENGTH_LONG).show();
                    return;
                }
    ...
    private void initCustomListView(){
        chatlist = DataMgr.getInstance().getNewChatInfoWithLimite(ConstantValue.GET_CHATINFO_MAX_COUNT, childid);
        downloadImgeJob = new DownloadImgeJob();
        getTeacherInfoJob = new GetSenderInfoJob();
        adapter = new NewChatListAdapter(this, chatlist, downloadImgeJob, getTeacherInfoJob);
    ...
    private boolean runGetChatTask(long from, long to, int type){
        ...
        if(getChatJob == null || getChatJob.isDone()){
            getChatJob = new GetChatJob(myhandler, ConstantValue.GET_CHATINFO_MAX_COUNT, from, to, type, childid);
            getChatJob.execute();
        } else{
            bret = false;
    ...
    private List<NewChatInfo> getChatFromLocal(){
        if(chatlist != null && !chatlist.isEmpty()){
            return DataMgr.getInstance().getNewChatInfoWithLimite(ConstantValue.GET_CHATINFO_MAX_COUNT,
                                                                  chatlist.get(0).getTimestamp(), childid);
        }
        return DataMgr.getInstance().getNewChatInfoWithLimite(ConstantValue.GET_CHATINFO_MAX_COUNT, childid);
    }
            ...
            public void handleMessage(Message msg){
                ...
                switch(msg.what){
                    ...
                    case EventType.GET_CHAT_FAIL:
                        Toast.makeText(NewChatActivity.this, "获取数据失败", Toast.LENGTH_SHORT).show();
                        break;
                    default:
                        break;
                }
    ...
    private void handleSendChat(){
        List<NewChatInfo> list = MyApplication.getInstance().getTmpNewChatList();
    ...
    private abstract class MyClickListner implements android.view.View.OnClickListener{
        @Override
        public void onClick(View v){
            if(!Utils.isNetworkConnected(NewChatActivity.this)){
                Toast.makeText(NewChatActivity.this, R.string.net_error, Toast.LENGTH_SHORT).show();
                return;
            }
