...
public class UpdateActivity extends UmengStatisticsActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		initHandler();
		initView();
		savepath = new File(Utils.getSDCardFileDir(Utils.APP_DIR_TMP), TMP_APK_NAME).getAbsolutePath();
	}
	...
	private void initBtn() {
		...
		updateBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				String last_update_url = DataUtils.getProp(JSONConstant.LAST_UPDATE_URL);
				if (current_apk_url.equals(last_update_url)) {
					File file = new File(savepath);

					if (file.exists() && file.length() == Long.valueOf(DataUtils.getProp(JSONConstant.FILE_SIZE))) {
	...
	private void downloadApkBySelf() {
		...
		MyThreadPoolMgr.getGenericService().submit(new Runnable() {
			...
			@Override
			public void run() {
				HttpClientHelper.downloadFile(current_apk_url, savepath, new DownloadFileListener() {
					@Override
					...
					public void onComplete() {
						DataUtils.saveProp(JSONConstant.LAST_UPDATE_URL, current_apk_url);
	...
	private long getFileSize() {
		long size = 0;
		try {
			size = Long.parseLong(DataUtils.getProp(JSONConstant.FILE_SIZE));
