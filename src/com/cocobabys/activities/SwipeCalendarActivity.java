...
public class SwipeCalendarActivity extends UmengStatisticsActivity {
	...
	private DateWidgetDayCell updateCalendar() {
		...
		for (int i = 0; i < days.size(); i++) {
			...
			dayCell.setFont(calendarFont.getCalendarDateFont(),
					calendarFont.getSwipeFont());
	...
	private SwipeInfoOnCalendar getRecord(Calendar calendar) {
		String format = InfoHelper.getYearMonthDayFormat().format(
				calendar.getTime());
	...
	private DateWidgetDayCell.OnItemClick mOnDayCellClick = new DateWidgetDayCell.OnItemClick() {
		public void OnClick(DateWidgetDayCell item) {
			if (!item.getRecord().isEmpty()) {
				...
				String format = InfoHelper.getYearMonthDayFormat().format(date);
