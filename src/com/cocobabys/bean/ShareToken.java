...
public class ShareToken {
	...
	public Bitmap getBitmap() {
		...
		// 如果都为空，返回默认图标
		if (bt == null) {
			bt = BitmapFactory.decodeResource(MyApplication.getInstance()
					.getResources(), R.drawable.logo_weixin);
		}
	...
	private Bitmap getBitmapImpl(List<String> localUrls) {
		...
		if (!localUrls.isEmpty()) {
			...
			try {
				bt = Utils.getLoacalBitmap(url, ConstantValue.NAIL_ICON_HEIGHT,
						ConstantValue.NAIL_ICON_WIDTH);
			} catch (Throwable e) {
				e.printStackTrace();
			}
		}
	...
	public String toString() {
		return "ShareToken [id=" + id + ", token=" + token + ", title=" + title
				+ ", content=" + content + "]";
	}
