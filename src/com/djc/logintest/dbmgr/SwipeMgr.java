...
class SwipeMgr {
    ...
    long addData(SwipeInfo info) {
        ...
        return writableDatabase.insertWithOnConflict(SqliteHelper.SWIPE_TAB, null, values,
    ...
    void addDataList(List<SwipeInfo> list) {
        ...
        for (SwipeInfo info : list) {
            ContentValues values = buildInfo(info);
            writableDatabase.insertWithOnConflict(SqliteHelper.SWIPE_TAB, null, values,
                    SQLiteDatabase.CONFLICT_IGNORE);
        }
        // 数据插入操作循环
    ...
    SwipeInfo getDataByID(int id) {
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM " + SqliteHelper.SWIPE_TAB + " WHERE "
                + SwipeInfo.ID + " = " + id, null);
    ...
    SwipeInfo getDataByTimeStamp(long timestamp) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + SqliteHelper.SWIPE_TAB + " WHERE "
                + SwipeInfo.TIMESTAMP + " = " + timestamp, null);
        List<SwipeInfo> list = getDataList(cursor);
        return list.isEmpty() ? null : list.get(0);
    }
    ...
    String getLastestSwipeIn(String date, String childid) {
        ...
        String sql = "SELECT * FROM " + SqliteHelper.SWIPE_TAB + " WHERE " + SwipeInfo.TYPE + " = "
                + JSONConstant.NOTICE_TYPE_SWIPECARD_CHECKIN + " AND " + SwipeInfo.CHILD_ID
                + " = '" + childid + "' AND " + SwipeInfo.TIMESTAMP + " >= " + startLimitTime
                + " AND " + SwipeInfo.TIMESTAMP + " < " + endLimitTime + " ORDER BY "
    ...
    String getLatestSwipeOut(String date, String childid) {
        ...
        long endLimitTime = startLimitTime + 24L * 60 * 60 * 1000L;
        Cursor cursor = db.rawQuery("SELECT * FROM " + SqliteHelper.SWIPE_TAB + " WHERE "
                + SwipeInfo.TYPE + " = " + JSONConstant.NOTICE_TYPE_SWIPECARD_CHECKOUT + " AND "
                + SwipeInfo.CHILD_ID + " = '" + childid + "' AND " + SwipeInfo.TIMESTAMP + " >= "
                + startLimitTime + " AND " + SwipeInfo.TIMESTAMP + " < " + endLimitTime
                + " ORDER BY " + SwipeInfo.TIMESTAMP + " DESC LIMIT 1", null);
        List<SwipeInfo> list = getDataList(cursor);
        return list.isEmpty() ? "" : list.get(0).getFormattedTime();
    }
    ...
    List<SwipeInfo> getAllSwipeCardNotice(String date, String childid) {
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM " + SqliteHelper.SWIPE_TAB + " WHERE "
                + SwipeInfo.CHILD_ID + " = '" + childid + "' AND " + SwipeInfo.TIMESTAMP + " >= "
                + startLimitTime + " AND " + SwipeInfo.TIMESTAMP + " < " + endLimitTime
                + " ORDER BY " + SwipeInfo.TIMESTAMP + " DESC", null);
