package com.djc.logintest.upload.qiniu.utils;
...
public class MultipartEntity extends AbstractHttpEntity {
	...
	public void writeTo(OutputStream outputStream) throws IOException {
		...
		if (mNotify != null)
			mNotify.onProcess(writed, getContentLength());
	...
	class FileInfo {
		...
		public long length() {
			return fileTpl.length() - 2 * 4 + mBoundary.length()
					+ mIsa.length() + 2 + mField.getBytes().length
					+ mContentType.length() + mFilename.getBytes().length;
		}

		public void writeTo(OutputStream outputStream) throws IOException {
			byte[] data = String.format(fileTpl, mBoundary, mField, mFilename,
					mContentType).getBytes();
			outputStream.write(data);
			outputStream.flush();
			writed += data.length;
			if (mNotify != null)
			...
			while (index < length) {
				int readLength = (int) StrictMath.min((long) blockSize,
						mIsa.length() - index);
