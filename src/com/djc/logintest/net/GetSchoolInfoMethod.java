...
public class GetSchoolInfoMethod {

    private GetSchoolInfoMethod() {
    }

    public static GetSchoolInfoMethod getGetAuthCodeMethod() {
        return new GetSchoolInfoMethod();
    }

    public int checkSchoolInfo() {
        ...
        if (result.getResCode() == HttpStatus.SC_OK) {
            ...
            try {
                ...
                if (errorcode == 0) {
                    ...
                    long oldtimestamp = schoolInfo.getTimestamp().equals("") ? -1 : Long
                            .parseLong(schoolInfo.getTimestamp());
                    // 信息没有更新，到此结束
    ...
    private int handleGetSchoolInfoResult(HttpResult result) {
        ...
        if (result.getResCode() == HttpStatus.SC_OK) {
            ...
            try {
                ...
                // 登录成功，保存学校动态信息
                if (errorcode == 0) {
                    String content = jsonObject.getString(JSONConstant.SCHOOL_INFO);
                    JSONObject schoolObj = new JSONObject(content);
                    SchoolInfo newInfo = SchoolInfo.jsonObjToChildInfo(schoolObj);
                    //单独解析school_id
                    DataMgr.getInstance().updateSchoolInfo(DataMgr.getInstance().getSchoolID(),
                            newInfo);
