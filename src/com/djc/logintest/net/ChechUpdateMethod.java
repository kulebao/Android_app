...
public class ChechUpdateMethod {
    ...
    public int chechUpdate(int versionCode) {
    ...
    private int handle(HttpResult result) {
        ...
        if (result.getResCode() == HttpStatus.SC_OK) {
            try {
                ...
                // 保存版本更新信息
                if (errorcode == 0) {
                    String updateUrl = jsonObject.getString(JSONConstant.UPDATE_URL);
                    String updateContent = jsonObject.getString(JSONConstant.UPDATE_CONTENT);
                    String versionName = jsonObject.getString(JSONConstant.UPDATE_VERSION_NAME);
                    ...
                    Utils.saveProp(JSONConstant.UPDATE_VERSION_NAME, versionName);
